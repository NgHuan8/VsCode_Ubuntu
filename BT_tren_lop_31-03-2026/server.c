...
// Quản lý trạng thái từng client
...
// Hàm tách tên và chữ cái đầu (VD: Nguyen Duc Huan -> huan, nd)
void process_name(char *full_name, char *first_name, char *initials) {
    ...
    if (count > 0) {
        ...
        for (int i = 0; i < count - 1; i++) {
            initials[idx++] = tolower(words[i][0]);
        }
        initials[idx] = '\0';
    }
}
...
int main() {
    ...
    printf("Server HUST Email dang chay tai port %d...\n", PORT);
                ...
                if (valread <= 0) {
                    ...
                    if (clients[i].state == WAIT_NAME) {
                        ...
                        if (strlen(clean_mssv) >= 8) {
                            ...
                            // Logic: 20235338 -> lấy '23' (index 2,3) và '5338' (4 số cuối)
                            char year_code[3] = {clean_mssv[2], clean_mssv[3], '\0'};
                            char last_four[5];
                            strcpy(last_four, &clean_mssv[strlen(clean_mssv)-4]);

                            snprintf(email, sizeof(email), 
                                     "Ket qua: %s.%s%s%s@sis.hust.edu.vn\n", 
                                     f_name, inits, year_code, last_four);
                            send(sd, email, strlen(email), 0);
                        } else {
                            char *err = "Loi: MSSV khong hop le (can 8 so).\n";
                            send(sd, err, strlen(err), 0);
                        }
                        
                        char *reset = "--- Hoan thanh ---\nNhap Ho ten moi: ";
                        send(sd, reset, strlen(reset), 0);
                        clients[i].state = WAIT_NAME;
                    }
                }
            }
